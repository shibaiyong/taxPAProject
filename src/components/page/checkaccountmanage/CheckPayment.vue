...
export default {
  ...
  methods: {
    ...
    handleExport(){
      ...
      idsStr = ids.join(',')
      if(idsStr){
        window.open('http://localhost:8088/export/exportReconciliationResultList?ids='+idsStr+'&reconciliationType=2','_self')
      }else{
        window.open('http://localhost:8088/export/exportReconciliationResultList?reconciliationType=2','_self')
      }
    },
